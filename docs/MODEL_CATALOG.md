# Model Catalogue — v0.2.0

This catalogue connects scientific model names to implementation status, source citations and validation evidence. A listed model is not automatically executable or validated outside its stated domain.
...
## Reference-only detailed mechanisms

CRECK/Ranzi, Bio-CPD/CPD and literature polymer mechanisms remain reference-only unless the exact executable source and redistribution terms are explicitly qualified. Use the cited DOI/source for legitimate access.
